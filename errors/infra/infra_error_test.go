...
func TestInfraErrorsAs(t *testing.T) {
	...
	// Test inheritance using IsInfrastructureError method
	// This is the recommended way to check if an error is an infrastructure error
	assert.True(t, dbErr.IsInfrastructureError(), "DatabaseError.IsInfrastructureError() should return true")
	assert.True(t, networkErr.IsInfrastructureError(), "NetworkError.IsInfrastructureError() should return true")
	assert.True(t, externalErr.IsInfrastructureError(), "ExternalServiceError.IsInfrastructureError() should return true")

	// Direct access to the embedded InfrastructureError also works
	assert.NotNil(t, dbErr.InfrastructureError, "DatabaseError.InfrastructureError should not be nil")
	assert.NotNil(t, networkErr.InfrastructureError, "NetworkError.InfrastructureError should not be nil")
	assert.NotNil(t, externalErr.InfrastructureError, "ExternalServiceError.InfrastructureError should not be nil")
}
