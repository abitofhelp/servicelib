...
func TestTemperature_CompareTo(t *testing.T) {
	temp1, _ := NewTemperature(25, Celsius)
	temp2, _ := NewTemperature(77, Fahrenheit) // 25°C = 77°F
	temp3, _ := NewTemperature(30, Celsius)

	if temp1.CompareTo(temp2) != 0 {
		t.Errorf("Expected 25°C to be equal to 77°F")
	}

	if temp1.CompareTo(temp3) != -1 {
		t.Errorf("Expected 25°C to be less than 30°C")
	}

	if temp3.CompareTo(temp1) != 1 {
		t.Errorf("Expected 30°C to be greater than 25°C")
	}
}

func TestTemperature_Validate(t *testing.T) {
	tests := []struct {
		name        string
		temp        Temperature
		expectError bool
	}{
		{"Valid Celsius", Temperature{25, Celsius}, false},
		{"Valid Fahrenheit", Temperature{77, Fahrenheit}, false},
		{"Valid Kelvin", Temperature{298.15, Kelvin}, false},
		{"Invalid Unit", Temperature{25, TemperatureUnit("X")}, true},
		{"Below Absolute Zero Celsius", Temperature{-274, Celsius}, true},
		{"Below Absolute Zero Fahrenheit", Temperature{-460, Fahrenheit}, true},
		{"Below Absolute Zero Kelvin", Temperature{-1, Kelvin}, true},
	}
